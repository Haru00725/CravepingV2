...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabase.from("cafes").insert(body).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  return NextResponse.json({ cafe: data }, { status: 201 });
}

// GET /api/cafes/qr?cafe_slug=brew-lab&table_id=table-1
export async function generateQR(req: NextRequest) {
  const { searchParams } = new URL(req.url);
